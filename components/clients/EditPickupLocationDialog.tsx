...
import type { CreatePickupLocationPayload, PickupLocation } from "@/lib/types/pickup-location";

const FORM_ID = "edit-pickup-location-form";
...
interface EditPickupLocationDialogProps {
  clientId: number;
...
export function EditPickupLocationDialog({
  clientId,
  location,
  isOpen,
  onClose,
  onSuccess,
}: EditPickupLocationDialogProps) {
  const formRef = useRef<HTMLFormElement>(null);
  const [values, setValues] = useState(locationToFormValues(location ?? ({} as PickupLocation)));
  ...
  useEffect(() => {
    ...
    setValues(locationToFormValues(location));
    setError(null);
    setSubmitting(false);
  ...
  }, [clientId, isOpen, location]);

  const setField = (
    field: keyof ReturnType<typeof locationToFormValues>,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `/api/clients/${clientId}/pickup-locations/${location.pickupLocationId}`,
        {
          method: "PUT",
      ...
      if (!response.ok) {
        const errorMessage =
          ...
          text ||
          `Failed to update pickup location (${response.status})`;
      ...
      setError(err instanceof Error ? err.message : "Failed to update pickup location");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <AppDialog
      isOpen={isOpen}
      onClose={onClose}
      title="Edit Client Pickup Location"
      titleId="edit-pickup-location-title"
      maxWidth="3xl"
