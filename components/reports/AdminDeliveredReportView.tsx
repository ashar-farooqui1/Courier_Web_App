...
export default function AdminDeliveredReportView() {
  ...
  const deliveredOrders = useMemo(
    () => orders.filter((order) => normalizeOrderStatusValue(order.status) === "Delivered"),
    [orders]
  );
  ...
  }, [deliveredOrders, appliedFilters, selectedClient, tableSearch]);
  ...
  const handleExport = () => {
    ...
    try {
      ...
      exportOrdersToCsv(filteredOrders, `delivered-orders-${today}.csv`);
    } finally {
      setExporting(false);
    }
  };
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={tableHeaders.length + 1} className="py-20 text-center">
                    <p className="text-slate-300 italic text-sm font-medium">No delivered orders found</p>
                  </td>
                </tr>
              ) : (
                pagedOrders.map((order) => (
                  <tr key={order.orderId} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4 whitespace-nowrap">{order.customerPhone || "—"}</td>
                    ...
                    <td className="p-4 whitespace-nowrap">
                      <span className="px-2 py-1 rounded bg-emerald-500 text-white text-[9px] font-bold uppercase">
                        Delivered
                      </span>
                    </td>
                    <td className="p-4 whitespace-nowrap">
