...
import { createDeliverySheet } from "@/lib/api/order";
...
/** Proxies POST /api/Order/CreateDeliverySheet */
export async function POST(request: Request) {
  ...
  try {
    const { message, data } = await createDeliverySheet(payload, token);
    ...
    const message = error instanceof Error ? error.message : "Failed to create delivery sheet";
