...
import { removeOrdersFromLoadsheet } from '@/lib/api/loadsheet';
...
/** Proxies POST /api/Order/RemoveOrdersFromLoadsheet */
export async function POST(request: Request) {
  ...
  try {
    const message = await removeOrdersFromLoadsheet({ loadsheetId, orderIds }, token);
    ...
    const message =
      error instanceof Error ? error.message : 'Failed to remove orders from loadsheet';
